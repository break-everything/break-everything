...
function getKind(tool: Tool): ToolKind {
  return tool.tool_kind === "web" ? "web" : "download";
}
...
export default function ToolAccessLinks({ tool, variant, onLinkClick }: ToolAccessLinksProps) {
  const kind = getKind(tool);
  const primaryHref = kind === "web" ? tool.web_url || "" : tool.download_url;
  const primaryLabel = kind === "web" ? "Open web app" : "Download";
  const primaryIcon = kind === "web" ? globeIcon : downloadIcon;
